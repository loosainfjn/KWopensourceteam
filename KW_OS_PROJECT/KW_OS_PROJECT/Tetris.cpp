...
void Tetris::reset_main(void) { //게임판을 초기화  
...
void Tetris::reset_main_cpy(void) { //main_cpy를 초기화 
...
void Tetris::draw_map(void) { //게임 상태 표시를 나타내는 함수  
...
void Tetris::draw_main(void) { //게임판 그리는 함수 
...
void Tetris::new_block(void) { //새로운 블록 생성  
...
int Tetris::check_crush(int bx, int by, int b_rotation) { //지정된 좌표와 회전값으로 충돌이 있는지 검사 
...
void Tetris::move_block(int dir) { //블록을 이동시킴 
...
void Tetris::check_level_up(void) {
...
void Tetris::check_game_over(void) {
...
void Tetris::pause(void) { //게임 일시정지 함수 
